...
def main():
    ...
    while(not flag.exit()):
        try:
            msg_id, message = socket.recv_multipart(flags=zmq.NOBLOCK)

            bit_count = bit_count + (len(message) * 8)
            msg_count = msg_count + 1

            if(msg_id == b"AVG"):
                msg_avg_count = msg_avg_count + 1
                ch_square = []
                ch_sum = []
                ch_sample_count = []
                for n in range(16):
                ...
                msg_stat_count = msg_stat_count + 1
                ...
                msg_adc_count = msg_adc_count + 1

                # Unpack message
                max_size = 32786
                # try:
                #     uncompressed = lz4.block.decompress(message, uncompressed_size=max_size)
                #     uncompressed_bit_count = uncompressed_bit_count + (len(uncompressed) * 8)
                ...
                num_words, num_pkt, id, ts = struct.unpack_from("<HIxBQ", uncompressed, 0)
                num_samples = num_words - 1

                if(num_pkt != last_pkt + 1) and (last_pkt != 0):
                    dropped_pkts = dropped_pkts + (num_pkt - (last_pkt + 1))
                    print("Streaming Packet Dropped")

                last_pkt = num_pkt

                n = 0
                ch0, ch1 = struct.unpack_from("<ii", uncompressed, 16 + (n * 8))
                ch1_data = ch1 >> 14
                ch0_data = ch0 >> 14
                ...
                factor = 2 if (ch0_sel == ch1_sel) else 1
                time_between_ts = ((num_samples * factor) * TS_CONVERSION * stream_div)
                if(last_ts != 0):
                    if((ts - last_ts) != time_between_ts):
                        missed_ts = missed_ts + 1
                        print(str((ts - last_ts) / time_between_ts))

                last_ts = ts

                # Every 'sample' is two channels of data
                sample_count = sample_count + (num_samples * factor)
                # print(num_samples * factor)

                sample_file.write(uncompressed)
        ...
        if (delta >= RATE):
            start_time = time.time()
            print("\x1b[2KRate " + f"{(bit_count / delta / 1000000):.3f}" + " mbit/s (" + f"{(uncompressed_bit_count / delta / 1000000):.3f}" + ") [" + f"{int((msg_count) / delta):d}" + "/" + f"{int((msg_adc_count) / delta):d}" + "/" + f"{int((msg_avg_count) / delta):d}" + "/" + f"{int((msg_stat_count) / delta):d}" + "," +  f"{(int(sample_count) / delta / 1000000):.3f}" + " mSamples/s, dropped: " + f"{(dropped_pkts / delta):.3f}" + " missed: " + f"{(missed_ts / delta):.3f}" + "]", end='\r')
            bit_count = 0
            uncompressed_bit_count = 0
            msg_count = 0
            msg_adc_count = 0
